...
void InputCapture::startCapture() {
    ...
    while (true) {
        ...
        // 自动录满 10秒也结束（可选）
        if (chrono::duration_cast<chrono::seconds>(now - startTime).count() >= 10) {
            cout << " 已录制 10秒,停止" << endl;
            destroyWindow("Recording...");
            break;
        }
    }
...
vector<cv::Mat>& InputCapture::getFrames() {
    return frames;
}

cv::VideoCapture& InputCapture::getCapture() {
    return cap;
}
